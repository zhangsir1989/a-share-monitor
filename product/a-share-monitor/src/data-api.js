...
/**
 * 获取沪深成交量数据（使用 MyData 实时指数接口）
 */
async function fetchMarketVolume() {
  try {
    // 使用 MyData 实时指数接口获取上证指数和深证成指的实时数据
    const [shResp, szResp] = await Promise.all([
      axios.get(`https://api.mairuiapi.com/hsindex/real/time/000001.SH/${MYDATA_LICENCE}`, { timeout: 10000 }),
      axios.get(`https://api.mairuiapi.com/hsindex/real/time/399001.SZ/${MYDATA_LICENCE}`, { timeout: 10000 })
    ]);
    ...
    const shAmountYuan = shData?.cje || 0;  // 成交额（元）
    const szAmountYuan = szData?.cje || 0;  // 成交额（元）
    ...
    console.error('获取沪深成交量失败 (MyData 实时):', e.message);
    return null;
  }
}
